...
const Cart=()=> {
  const { cart, setCart, user ,setUser,setOrders,orders} = useContext(MyContext);
...
setTotal(sum)
  ...
  return (
    <div>
      {placedOrder? <h2>Thanks for placing order: </h2> :<div>
        {cart.map((meal) => {
          return (
            <div key={meal._id}>
      ...
      <div className="total"> {cart.length>0 && <h2> Total :{total} </h2>} </div>
      <button onClick={placeOrder}>checkout</button>
      <h3>{message}</h3>
        <h3>Address: </h3>
      <form onSubmit={getAddress}>
        <label>House No.<input type="number" name="hn" min={1} /></label><br />
        <label>Street No.<input type="number" name="stn" /></label><br />
        <label>City.<input type="text" name="city" /></label><br />
        <label>Postal Code.<input type="number" name="pc" /></label><br />
